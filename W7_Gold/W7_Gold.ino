  ...
  if (run) {
    ...
    if (distance > stopDist && distance != 0) {
      ...
      if (digitalRead(LEYE) != line && digitalRead(REYE) != line) {
        forward();
      } else if (digitalRead(LEYE) == line) {
        bankl();
      } else if (digitalRead(REYE) == line) {
        bankr();
      } else if (digitalRead(LEYE) == line && digitalRead(REYE) == line) {
        //PATH SELECTION CODE GOES HERE
        forward();
        delay(50);
        if (tagID == 3) {
          while(digitalRead(REYE) == line){
            bankl();
          }
        }
        else if (tagID == 4) {
